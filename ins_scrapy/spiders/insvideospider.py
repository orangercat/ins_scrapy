...
class InsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        json_data = json.loads(
            "".join(re.findall(r'window._sharedData = (.*);', javascript)))
        '''
        node sample
        {'code': 'BRgugwTgkDs',
         'comments': {'count': 0},
        ...
        taken_at = json_data['entry_data'][
            'LocationsPage'][0]['location']['id']
        ...
        for node in top_post_nodes_data:
            ...
            if node['is_video'] and node['comments']['count'] + node['likes']['count'] > 50:
                ...
                # print(node['code'])
                item['url'] = 'https://www.instagram.com/p/' + \
                    node['code'] + '/?taken-at=' + taken_at
